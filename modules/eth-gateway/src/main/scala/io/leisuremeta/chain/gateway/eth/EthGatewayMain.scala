...
import org.web3j.abi.{EventEncoder, FunctionEncoder, FunctionReturnDecoder, TypeReference}
...
object EthGatewayMain extends IOApp:
  ...
  extension[A] (logResult: LogResult[A])
    def toTransferTokenEvent: TransferTokenEvent =
      ...
      val typeRefAddress: TypeReference[Address] = new TypeReference[Address]() {}
      ...
      val from: Address = FunctionReturnDecoder.decodeIndexedValue[Address](topics(1), typeRefAddress).asInstanceOf[Address]
      val to: Address = FunctionReturnDecoder.decodeIndexedValue[Address](topics(2), typeRefAddress).asInstanceOf[Address]
      val amount = FunctionReturnDecoder.decode(log.getData, List(typeRefUint256).asJava).asScala.head.asInstanceOf[Uint256].getValue
  ...
  def getTransferTokenEvents(
    ...
    val TransferEvent = new Event("Transfer",
      Arrays.asList(
  ...
  def transferToken(
    ...
    val params = new ArrayList[Type[_]]()
    params.add(new Address(toAddress))
    params.add(new Uint256(amount.bigInteger))

    val returnTypes = Collections.emptyList[TypeReference[_]]()
  ...
  def submitTx(lmAddress: String, account: Account, keyPair: KeyPair, tx: Transaction) =
...
//    submitTx(lmAddress, account, keyPair, addAccounts)
  ...
  def mintLM(lmAddress: String, keyPair: KeyPair, toAccount: Account, amount: BigInt): IO[Unit] = IO{
  ...
  def findAccountByEthAddress(lmAddress: String, ethAddress: String): IO[Option[Account]] = IO {
    val response = basicRequest
      .response(asStringAlways)
  ...
  def checkDepositAndMint(web3j: Web3j, lmAddress: String, ethContract: String, gatewayEthAddress: String, keyPair: KeyPair, startBlockNumber: BigInt): IO[Unit] =
    for
      ...
      depositEvents = events.filter(_.to.toLowerCase === gatewayEthAddress.toLowerCase)
      toMints <- depositEvents.toList.traverse{ event =>
        ...
        findAccountByEthAddress(lmAddress, event.from).map{
          (accountOption: Option[Account]) => accountOption.map{ account =>
            (account, amount)
          }
        }
      }
  ...
  def run(args: List[String]): IO[ExitCode] =
    ...
    for
      ...
      _ <- web3Resource(gatewayConf.ethAddress).use{ web3 =>
        ...
        val keyPair = CryptoOps.fromPrivate(BigInt(gatewayConf.ethPrivate.drop(2), 16))
