...
object ExternalApiService:
  def requestCoinMarket[F[_]: Async](backend: SttpBackend[F, Any]): EitherT[F, String, LmPrice] =
    ...
    get[F, LmPrice](backend) {
      uri"http://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?id=${lmTokenId}"
    }.leftMap { msg =>
  ...
  def getLmPrice[F[_]: Async](backend: SttpBackend[F, Any]): EitherT[F, String, Option[Data]] =
    for 
      ...
      result <- if res.status.error_code == 0 then EitherT.pure(Some(res.data))
      else {
  ...
  def get[F[_]: Async, A: io.circe.Decoder](
    ...
    EitherT {
      basicRequest
        .header("X-CMC_PRO_API_KEY", "b78d5940-b63d-4d90-99fc-58f65be72fa0")
        .get(uri)
        .send(backend)
        .map { response =>
          for
