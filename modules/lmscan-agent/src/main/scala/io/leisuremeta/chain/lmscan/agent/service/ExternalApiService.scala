...
object ExternalApiService:
  def requestCoinMarket[F[_]: Async](backend: SttpBackend[F, Any]): EitherT[F, String, Double] =
    ...
    for
      coinMarketResOpt <- get[F, LmPrice](backend) {
      ...
      result <- coinMarketResOpt match 
          case Some(res) if res.status.error_code == 0 =>
            scribe.info(s"getLmPrice response: $res")
            EitherT.pure(res.data("20315").quote.USD.price)
          case _ => 
            for
              price <- SummaryService.getLastSavedLmPrice[F].flatMap {
                case Some(value) => EitherT.pure(value)
                case None => EitherT.pure(0.0)
              }
            yield (price)
    yield result
