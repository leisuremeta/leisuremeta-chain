...
import crypto.{KeyPair, MerkleTrie, Recover, Sign, Signature}
...
object GossipDomain {
  ...
  object UpdateState {
    def apply[F[_]](implicit us: UpdateState[F]): UpdateState[F] = us
  }

  def onNewTx[F[_]: Monad](gossip: LocalGossip, tx: Signed.Tx)(implicit
    ...
    for {
      isSupportings <- EitherT.right[String](
        headers.traverse{
          case (blockHash, (block@_, state)) =>
            updateState(state, tx).value.map{
              case Right(_) => true
              case Left(msg) =>
                scribe.debug(s"block $blockHash does not support tx ${tx.toHash}: $msg")
                false
            }
        }
      )
  ...
  def generateNewBlockSuggestion[F[_]: Monad](
    ...
    if (candidateTxs.isEmpty && bestBlock.transactionHashes.isEmpty)
      EitherT.leftT[F, (Block.BlockHash, Block)](
        "No transactions to include in new block"
      )
    else {
      implicit val idNodeStore: NodeStore[cats.Id, Signed.TxHash, Unit] =
        Kleisli.pure(None)
      for {
        (state, txSet) <- candidateTxs.toList.foldM(
          ...
          (for {
            ...
            state1 <- updateState(state, tx)
          } yield (state1, txSet + txHash)).recover { _ => (state, txSet) }
        }
        txState = txSet.toList.foldLeft(
          MerkleTrie.MerkleTrieState.empty[Signed.TxHash, Unit]
        ) { (state, txHash) =>
          MerkleTrie
            .put[cats.Id, Signed.TxHash, Unit](txHash.bits, ())
            .runS(state)
            .value
            .getOrElse(state)
        }
        header = Block.Header(
          ...
          namesRoot = state.namesState.root,
          tokenRoot = state.tokenState.root,
          balanceRoot = state.balanceState.root,
  ...
  def onNewBlockSuggestion[F[_]: Monad](
    ...
    val parentStateEither: Either[String, MerkleState] =
      if (block.header.parentHash === gossip.bestConfirmed._1) {
    ...
    for {
      ...
      newState    <- txs.foldLeftM(parentState)(updateState(_, _))
      ...
      val bestBlock1 =
        if (isLocal) Some((blockHash, (block, newState))) else gossip1.bestBlock
  ...
  private def checkBlockValidityAndGetProviderIndex(
    ...
    providerAddress = Address.fromPublicKeyHash(publicKey.toHash)
    providerIndex <- params.nodeAddresses
      .find(_._2 === providerAddress)
      .map(_._1)
      .toRight(
        s"provider address is not found: $providerAddress"
      )
    ...
    _ <- Either.cond(
      ...
      s"block ${blockHash} has invalid authority:\n"
        + s"provider address: $providerAddress\n"
  ...
  private def isDescendant(
    ...
    if (parentHash === ancestorHash) true
    else
  ...
  private def isVotableBlockSuggestion(
    ...
    blockSuggestion.header.number.value > bestBlock.header.number.value
  }
  ...
  def onNewBlockVote(
    ...
    (block, state) <- getBlock(gossip, blockHash)
    _ <- Either.cond(
      block.votes =!= Set(sig),
      (),
    ...
    val bestBlock1 =
      if (nodeNo === params.localNodeIndex) Some((blockHash, (block, state)))
  ...
  def tryFinalizeBlockWithBlockHash(
      ...
      blockHash: Block.BlockHash,
      params: GossipParams,
  ): Either[String, (LocalGossip, List[(Block.BlockHash, Block)])] =
  ...
  private def finalizeBlockAndItsAncestors(
    ...
    if (gossip.bestConfirmed._1 === parentHash) {
      ...
      val blockSetToRemove =
        (for {
          ...
          if !blockHashSetToFinalize.contains(
            blockHash
      ...
      for {
        ...
        val bestBlock1 = gossip.bestBlock match {
          ...
          case Some((blockHash, (bestBlock, _))) =>
            if (isDescendant(gossip, bestConfirmed1._1, bestBlock.header)) {
  ...
  private def rebaseState(
    ...
    for {
      ...
      tokenState   <- state.tokenState.rebase(newBase.tokenState)
      balanceState <- state.balanceState.rebase(newBase.balanceState)
    } yield MerkleState(namesState, tokenState, balanceState)
  }
}
